...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        polygon_input = gr.Textbox(label="Polygon Coordinates (e.g., [(x1, y1), (x2, y2), ...])", placeholder="[(50, 50), (150, 50), (150, 150), (50, 150)]")

        return [pixels, mask_blur, inpainting_fill, direction, polygon_input]

    def run(self, p, pixels, mask_blur, inpainting_fill, direction, polygon_coords):
        initial_seed = None
        initial_info = None

        # Convert polygon string input to a list of tuples
        polygon = ast.literal_eval(polygon_coords)

        # Create a blank mask
        init_img = p.init_images[0]
        mask = Image.new("L", (init_img.width, init_img.height), "white")
        draw = ImageDraw.Draw(mask)

        # Draw the polygon on the mask
        draw.polygon(polygon, fill="black")

        # Convert polygon mask to rectangle (bounding box)
        bbox = mask.getbbox()  # Get the bounding box of the polygon
        if bbox:
            left, upper, right, lower = bbox
            # Create a rectangular mask
            rect_mask = Image.new("L", (init_img.width, init_img.height), "white")
            draw_rect = ImageDraw.Draw(rect_mask)
            draw_rect.rectangle([left, upper, right, lower], fill="black")
        else:
            rect_mask = mask  # If no bounding box, use the original mask
        ...
        # Set the mask for processing
        p.image_mask = rect_mask
