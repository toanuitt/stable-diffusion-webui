...
class EraserSetterRGB(object):
    ...
    def __call__(self, inst, eraser, rgb):
        return place_eraser_in_ratio_rgb(inst, eraser, rgb, self.min_overlap,
                                           self.max_overlap, self.min_cut_ratio,
                                           self.max_cut_ratio, 100)


def place_eraser_in_ratio_rgb(inst, eraser, rgb, min_overlap, max_overlap, min_ratio, max_ratio, max_iter):
    for i in range(max_iter):
        # print(i)
        # import time
        # start_t = time.time()
        shift_eraser, ratio, shift_rgb = place_eraser_rgb(inst, eraser, rgb, min_overlap, max_overlap)
        # print('place_eraser_rgb_time', time.time() - start_t)
        # ipdb.set_trace()
        if ratio >= min_ratio and ratio < max_ratio:
            break
    return shift_eraser, shift_rgb


def place_eraser_rgb(inst, eraser, rgb, min_overlap, max_overlap):
    assert len(inst.shape) == 2
    assert len(eraser.shape) == 2
    assert min_overlap <= max_overlap
    h, w = inst.shape
    ...
    bbox = (int(offx * h), int(offy * h), w, h)
    shift_eraser = crop_padding(eraser, bbox, pad_value=(0,))
    shift_rgb = crop_padding(rgb, bbox, pad_value=(0,0,0))
    assert inst.max() <= 1, "inst max: {}".format(inst.max())
    assert shift_eraser.max() <= 1, "eraser max: {}".format(eraser.max())
    ratio = ((inst == 1) & (shift_eraser == 1)).sum() / float((inst == 1).sum() + 1e-5)
    return shift_eraser, ratio, shift_rgb