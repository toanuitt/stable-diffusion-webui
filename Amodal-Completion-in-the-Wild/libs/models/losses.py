...
class MaskWeightedCrossEntropyGaussian(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        pre_loss = 1 / 2 * (
            F.binary_cross_entropy(mean, target, reduction="none") / std.pow(2)
        ) + 1 / 2 * std.pow(2)

        loss = (
            self.inmask_weight * pre_loss[mask == 1].sum()
            + self.outmask_weight * pre_loss[mask == 0].sum()
        ) / (n * h * w + 1)
...
class MaskWeightedNLLExponential(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        pre_loss = torch.where(
            target == 1,
            1 / mean + 1 / 2 * mean.pow(2) - 3 / 2,
            1 / 2 * mean.pow(2),
        )
...
class MaskWeightedNLLGaussianBoundary2(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        std_loss = (
            self.inmask_weight * 2 * std_loss[std_target == 1].sum()
            + self.outmask_weight * std_loss[std_target == 0].sum()
        )
...
class MaskWeightedNLLGaussianBoundary(nn.Module):
    ...
    def forward(self, predict, target, mask):
        ...
        std_loss = (
            self.inmask_weight * std_loss[std_target == 1].sum()
            + self.outmask_weight * std_loss[std_target == 0].sum()
        )
